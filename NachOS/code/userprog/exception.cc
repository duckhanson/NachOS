	...
	case SyscallException:
		...
		case SC_Open:
			...
			val = kernel->machine->ReadRegister(4);
			{
				...
				kernel->machine->WriteRegister(2, (int)fileID); // fileID puts back to return
			}
			kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
			kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
			kernel->machine->WriteRegister(NextPCReg, kernel->machine->ReadRegister(PCReg) + 4);
		...
		case SC_Write:
			...
			fileID = (int)kernel->machine->ReadRegister(6);
			{
				...
				kernel->machine->WriteRegister(2, (int)numChar); // fileID puts back to return
			}
			kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
			kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
			kernel->machine->WriteRegister(NextPCReg, kernel->machine->ReadRegister(PCReg) + 4);
		...
		case SC_Read:
			...
			break;
		case SC_Close:
			DEBUG(dbgAddr, "File close\n");
			fileID = (int)kernel->machine->ReadRegister(4);
			status = SysClose(fileID);
			kernel->machine->WriteRegister(2, (int)status); // fileID puts back to return
			kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
			kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
			kernel->machine->WriteRegister(NextPCReg, kernel->machine->ReadRegister(PCReg) + 4);
			return;
